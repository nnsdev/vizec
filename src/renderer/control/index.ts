...
const audioMeterFill = document.getElementById('audio-meter-fill') as HTMLDivElement;
...
async function init() {
  ...
  console.log('window.vizecAPI:', window.vizecAPI);
  ...
  try {
    ...
    const [presetsData, screenSources, inputDevices, visualizationsData, stateData] = await Promise.all([
    ...
    presets = presetsData;
    // Combine screen sources with audio input devices
    audioSources = [
...
function updateUIFromState() {
  ...
  const orderRadios = document.querySelectorAll('input[name="rotation-order"]') as NodeListOf<HTMLInputElement>;
  ...
  // Display settings
  const bgRadios = document.querySelectorAll('input[name="background"]') as NodeListOf<HTMLInputElement>;
...
function updateVisualizationSettings() {
  ...
  const viz = createVisualization(currentState.currentVisualization);
  if (!viz) {
    vizSettingsContainer.innerHTML = '<p style="color: var(--text-secondary);">No settings available</p>';
    return;
  }
...
function setupEventListeners() {
  ...
  // Audio source selection - capture starts automatically
  ...
  const getRotationConfig = () => ({
    ...
    order: (document.querySelector('input[name="rotation-order"]:checked') as HTMLInputElement)?.value as 'sequential' | 'random',
  ...
  randomizeAllCheck.addEventListener('change', () => {
    // If randomize all is checked, also check randomize colors
  ...
  // Refresh audio sources button (on select focus)
