...
export class HexShield extends BaseVisualization {
  ...
  render(audioData: AudioData, _deltaTime: number): void {
    ...
    const sensitivityFactor = Math.pow(sensitivity / 2, 2);
    ...
    this.hexes.forEach((hex) => {
      ...
      const val =
        (frequencyData[hex.freqIndex % frequencyData.length] || 0) / 255;
      const threshold = 0.4 - sensitivityFactor * 0.25; // 0.4 at low sens, 0.15 at high sens

      if (val > threshold) {
        hex.active = Math.min(
          1.0,
          hex.active + (val - threshold) * sensitivityFactor * 0.8,
        );
      }

      if (
        bass > 0.7 - sensitivityFactor * 0.2 &&
        Math.random() > 0.97 - sensitivityFactor * 0.1
      ) {
        hex.active = Math.min(1.0, hex.active + 0.5 * sensitivityFactor);
      }
